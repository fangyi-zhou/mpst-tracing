...
type marking map[label][]tokenWithMultiplicity
...
func consumeResources(a []tokenWithMultiplicity, b []tokenWithMultiplicity) ([]tokenWithMultiplicity, []tokenWithMultiplicity, []tokenWithMultiplicity) {
	...
	for _, ael := range a {
		...
		for _, bel := range b {
			if ael.token == bel.token {
				toConsume = bel.multiplicity
				break
			}
		}
		if toConsume > 0 {
			...
			if ael.multiplicity > toConsume { //if there are strictly more tokens than required
				eel := ael
				eel.multiplicity = ael.multiplicity - toConsume
				extra = append(extra, eel)

				cel := ael
				cel.multiplicity = toConsume
				consumed = append(consumed, cel)
			} else if ael.multiplicity == toConsume {
				consumed = append(consumed, ael) // all resources were consumed
			} else {
				cel := ael
				cel.multiplicity = ael.multiplicity
				consumed = append(consumed, cel)

				rel := ael
				cel.multiplicity = toConsume - ael.multiplicity
	...
	for _, bel := range b {
		...
		if stillRemain {
			remaining = append(remaining, bel)
		}
	}
